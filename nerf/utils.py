...
class Trainer(object):
    def __init__(self,
                 ...
                 lr_scheduler_pose=None, # lr_scheduler_pose
        ...
        if lr_scheduler is None:
            ...
            self.lr_scheduler_pose = optim.lr_scheduler.LambdaLR(self.optimizer_pose, lr_lambda=lambda epoch: 1) # fake scheduler
        else:
            self.lr_scheduler = lr_scheduler(self.optimizer)
            self.lr_scheduler_pose = lr_scheduler_pose(self.optimizer)
    ...
    def train_step(self, data, save_guidance_path:Path=None):
        ...
        return pred_rgb, pred_depth, loss, do_rgbd_loss
    ...
    def post_train_step(self):
        ...
        self.scaler.unscale_(self.optimizer_pose)
    ...
    def train_one_epoch(self, loader, max_epochs):
        ...
        if self.pose_net is not None:
            print("pose model in training stage")
            self.pose_net.train()
        ...
        for data in loader:
            ...
            with torch.cuda.amp.autocast(enabled=self.fp16):
                ...
                # if self.pose_net is not None:
                #     if self.local_step in [1,2]:
                #         print("pose model in training stage")
                #         self.pose_net.train()
                #     else:
                #         print("pose model in eval stage")
                #         self.pose_net.eval()
                
                pred_rgbs, pred_depths, loss, do_rgbd_loss = self.train_step(data, save_guidance_path=save_guidance_path)
