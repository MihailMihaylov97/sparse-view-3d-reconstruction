...
from nerf.pose_predictor import LearnPose
...
if __name__ == '__main__':
    ...
    parser.add_argument('--simple_inference', action='store_true', help="test mode")
    ...
    parser.add_argument('--known_view_scale', type=float, default=1.5625, help="multiply --h/w by this for known view rendering")
    ...
    pose_net = LearnPose(num_cams=2, learn_R=True, learn_t=True, init_c2w=None).to(device)
    ...
    if opt.six_views:
        ...
        if opt.gui:
            ...
            if opt.save_mesh:
                trainer.save_mesh()
    elif opt.simple_inference:
        guidance = None
        
        trainer = Trainer(' '.join(sys.argv), 'df', opt, model, guidance, device=device, workspace=opt.workspace, fp16=opt.fp16, use_checkpoint=opt.ckpt)
        train_loader = NeRFDataset(opt, device=device, type='train', H=opt.H, W=opt.W, size=opt.dataset_size_test).dataloader(batch_size=1)

        pred_rgb, pred_depth, _ = trainer.test_step(train_loader._data.get_default_view_data())
        
        pred = pred_rgb[1].detach().cpu().numpy()
        pred = (pred * 255).astype(np.uint8)

        pred_depth = pred_depth[1].detach().cpu().numpy()
        pred_depth = (pred_depth - pred_depth.min()) / (pred_depth.max() - pred_depth.min() + 1e-6)
        pred_depth = (pred_depth * 255).astype(np.uint8)


        cv2.imwrite("rgb_2.png", cv2.cvtColor(pred, cv2.COLOR_RGB2BGR))
        cv2.imwrite("depth_2.png", pred_depth)
        
        exit()
        
    else:

        train_loader = NeRFDataset(opt, device=device, type='train', H=opt.h, W=opt.w, size=opt.dataset_size_train * opt.batch_size, pose_predictor=pose_net).dataloader()
        ...
        if opt.optim == 'adan':
            ...
            params = model.get_params(5 * opt.lr)
            optimizer = Adan(params, eps=1e-8, weight_decay=2e-5, max_grad_norm=5.0, foreach=False)
            optimizer_pose = Adan(pose_net.parameters(), eps=1e-8, weight_decay=2e-5, max_grad_norm=5.0, foreach=False)
        else: # adam
            # optimizer = lambda model: torch.optim.Adam(model.get_params(opt.lr), betas=(0.9, 0.99), eps=1e-15)
            params = model.get_params(5 * opt.lr)
            optimizer = torch.optim.Adam(params, eps=1e-8, weight_decay=2e-5)
            optimizer_pose = torch.optim.Adam(pose_net.parameters(), eps=1e-8, weight_decay=2e-5)
            

        if opt.backbone == 'vanilla':
            scheduler = lambda optimizer: optim.lr_scheduler.LambdaLR(optimizer, lambda iter: 0.1 ** min(iter / opt.iters, 1))
        else:
            scheduler = lambda optimizer: optim.lr_scheduler.LambdaLR(optimizer, lambda iter: 1) # fixedc
            scheduler_pose = lambda optimizer: optim.lr_scheduler.LambdaLR(optimizer_pose, lambda iter: 1) # fixedc
            # scheduler = lambda optimizer: optim.lr_scheduler.LambdaLR(optimizer, lambda iter: 0.1 ** min(iter / opt.iters, 1))
        ...
        trainer = Trainer(' '.join(sys.argv), 'df', opt, model, pose_net, guidance, device=device, workspace=opt.workspace, optimizer=optimizer, optimizer_pose=optimizer_pose, ema_decay=0.95, fp16=opt.fp16, lr_scheduler=scheduler, lr_scheduler_pose=scheduler_pose, use_checkpoint=opt.ckpt, scheduler_update_every_step=True)

        trainer.default_view_data = train_loader._data.get_default_view_data(pose_net)
        # trainer.default_view_data = train_loader._data.get_default_view_data_debug()
        ...
        if opt.gui:
            ...
            valid_loader = NeRFDataset(opt, device=device, type='val', H=opt.H, W=opt.W, size=opt.dataset_size_valid, pose_predictor = pose_net).dataloader(batch_size=1)
            test_loader = NeRFDataset(opt, device=device, type='test', H=opt.H, W=opt.W, size=opt.dataset_size_test, pose_predictor = pose_net).dataloader(batch_size=1)
