...
class _near_far_from_aabb(Function):
    ...
    def forward(ctx, rays_o, rays_d, aabb, min_near=0.2):
        ...
        # ctx.save_for_backward(rays_o, rays_d, nears, fars)
    ...
    #     # grad_nears = grad_nears.contiguous()
    #     # grad_fars = grad_fars.contiguous()
        
    #     # print(f"{grad_nears.shape}")
    #     # print(f"{grad_fars.shape}")
    #     rays_o, rays_d, nears, fars = ctx.saved_tensors
...
class _march_rays_train(Function):
    ...
    def forward(ctx, rays_o, rays_d, bound, density_bitfield, C, H, nears, fars, perturb=False, dt_gamma=0, max_steps=1024, contract=False):
        ...
        # ctx.save_for_backward(rays_o, rays_d, nears, fars)
        # ctx.dims = [M, N, bound, density_bitfield, C, H, dt_gamma, max_steps, contract, step_counter, noises]

        return xyzs, dirs, ts, rays

    # @staticmethod
    # @custom_bwd
    # def backward(ctx, grad_xyzs, grad_dirs, grad_ts, grad_rays):
    #     grad_xyzs = grad_xyzs.contiguous()
    #     grad_dirs = grad_dirs.contiguous()
    #     grad_ts = grad_ts.contiguous()
    #     grad_rays = grad_rays.contiguous()
        
    #     rays_o, rays_d, nears, fars = ctx.saved_tensors
    #     M, N, bound, density_bitfield, C, H, dt_gamma, max_steps, contract, step_counter, noises = ctx.dims
        
    #     # get_backend().march_rays_train(rays_o, rays_d, density_bitfield, bound, contract, dt_gamma, max_steps, N, C, H, nears, fars,grad_xyzs, grad_dirs, grad_ts, grad_rays, step_counter, noises)

    #     return rays_o, rays_d, None, None, None, None, None, grad_xyzs, grad_dirs, grad_ts, grad_rays
